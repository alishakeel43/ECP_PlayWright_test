...
test.describe("Davis Vision Plan Test", () => {
  test.setTimeout(0); // 10 minutes

  test.beforeAll(async () => {
    connection = await createConnection();
    storageStatePath = await loginAndSaveStorage(userEmail, password);
    testData = await fetchTestData(connection, userEmail, "Davis Vision");
  ...
  test("Run Davis Vision Plan Flow", async ({ browser }) => {
    if (!testData.length) {
      console.warn("⚠️ No test data found for Davis Vision.");
      return;
    }

    for (const data of testData) {
      // Unique subfolder for each iteration video
      const videoDir = path.join(__dirname, "../videos", data.collectionTitle.replace(/[^\w\s-]/g, "_"));
      ...
      try {
        ...
        console.error(
          `❌ Error in Davis Vision test for ${data.collectionTitle}:`,
          error.message
        );
